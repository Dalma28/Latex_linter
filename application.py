...
from rules_file import RulesClass
...
def main():
    ...
    while file_name.endswith(".tex") is False:
        ...
        file_name = input("Please type the name of the latex file that you want to edit: ")
    try:
        input_file = InputClass(file_name)
        input_file.reading_file()
        user_object = RulesClass(input_file.file_content)
        user_object.new_line()
        user_object.format_comment()
        user_object.blank_lines()
        user_object.tab_adding()
        result = OutputClass(user_object.lines_container,file_name)
        result.edited_file()
